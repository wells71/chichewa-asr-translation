...
**Models:**
- Automatic Speech Recognition: `zerolat3ncy/faster-whisper-medium-chichewa`
- Chichewa to English Translation: `zerolat3ncy/nllb-financial-nya-en`
- English to Chichewa Translation: `zerolat3ncy/nllb-financial-nya-en`

**Supported Languages:**
- Chichewa (Nyanja) - ISO 639-3: nya
...
with st.expander("System Status"):
    ...
    with col1:
        st.markdown("**ASR & Translation API**")
        if st.button("Check Status", key="check_asr"):
            try:
                response = requests.get(f"{ASR_API_URL}/", timeout=10)
                if response.status_code == 200:
                    st.success("Online")
                    with st.json(response.json()):
                        pass
                else:
    ...
    with col2:
        st.markdown("**English to Chichewa API**")
        if st.button("Check Status", key="check_translation"):
            try:
                response = requests.get(f"{EN_NYA_API_URL}/", timeout=10)
                if response.status_code == 200:
                    st.success("Online")
                    with st.json(response.json()):
                        pass
